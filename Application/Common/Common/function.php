 ...
 * $uid 设备ID
 */
function getUmeber_udata($uid){
    return $uRow = M("user")-> where(array('uid' => $uid) )-> field("username,account,gpscycle,level,createtime,validtime") ->find(); 
}
/**
 * 群组成员列表查询相关列表信息
 * $uid 设备ID
 */
function getStadata($uid){
    return $staRow = M("user_status")-> where(array('uid' => $uid) )-> field("online,note") ->find(); 
}

/**
 * 查询续约订单的账号个数
 */
function xyUserCount($blid,$bltype){
    if ($bltype == 1){
        echo $xuzLisr = M("bk_item")-> where(array('biblid' => $blid)) ->count();
    }else{
        $xuzLisr = M("bk_list")-> where(array('blid' => $blid)) -> field("blnum") ->find();
        echo $xuzLisr['blnum'];
    }
}
/**管理员列表
...
function getmaxData ($opid){
    ...
    $maxRows['quota'] = $quota;
    $maxRows['quotaed'] = $quotaed;
    $maxRows['vquota'] = $vquota;
    return $maxRows;
...
//订单列表获取批量续费账号
function getUserAccount($blid){
    $list = array();
    $ModifyDate = 0;
    $user = "";
    //$sql = "SELECT a.account, b.bitype FROM tab_user AS a INNER JOIN tab_bk_item AS b ON a.uid = b.biuid WHERE b.biblid = $blid";
    $userCnt = M("user a") -> join("INNER JOIN tab_bk_item AS b ON a.uid = b.biuid") -> where(array('b.biblid'=> $blid)) -> field('a.account, b.bitype') -> select();
    //echo M("user a") ->  getLastSql();
    if (!empty($userCnt)) {
        foreach ($userCnt as $v) {
            $account = $v['account'];
            $bitype = $v['bitype'];
            $user .= $account. ',';
            $ModifyDate = $bitype;
...
function usedNum($opid){
    ...
    switch ($nLevel)
    {
        case 0:
            ...
            echo $usednum = $curnum + $subnum; //本身的设备加上管理员下的设备
        case 1:
...
function getState($uid){
    $stateData = M('pause') -> where(array('uid' => $uid)) -> field('state') -> find();
...
function heaerNums($params)  //将opid传进
{
    ...
    $usednum = 0;
    $dbution = 0;
    $adRow = M('admain') -> where(array('opid' => $params)) -> field('level') -> find();
    ...
    $adRowdata = M('max') -> where(array('opid' => $params)) -> field('quota,quotaed') -> find();
    if (!empty($adRowdata))
    {
        $maxuser = $adRowdata['quota'];
        $dbution = $adRowdata['quotaed'];
    ...
    switch ($nLevel)
    {
        case 0:
            $tNumList = array();
            //-1级管理员
            $dataList = M('admain') ->where(array('manid' => $params))  -> field('opid') -> select();
            //dump($dataList);
            if (!empty($dataList))
            {
                foreach ($dataList as $key => $v) {
            ...
            //$tNumList = array_merge($mNumList,$zNumList); // 包括1级0级本身的设备和名下管理员的设备
            $tNumList[] = $params;  //加上0级本身的设备
            if (!empty($tNumList)) {
                $mwhere['manid'] = array('in',$tNumList);
                $usednum = M('user') ->where($mwhere) -> count();
            }else{
                $usednum = 0;
            }
            break;
